...
const Calculator = ({
  ...
  const [addressOut, setAddressOut] = useState(networkTokens[0].address);
  const [symbolOut, setSymbolOut] = useState(networkTokens[0].symbol);
  const [decimalsOut, setDecimalsOut] = useState(networkTokens[0].decimals);
  const [decimalsIn, setDecimalsIn] = useState(networkTokens[0].decimals);
  ...
  const dropdownHandlerOne = (selectedOption) => {
    ...
    // );
    console.log(selectedOption, "selected", chainId);
      ...
      case "first":
        if (amount > 0) {
          ...
          let amountsOut = await routerInstance.getAmountsOut(
            ethers.utils.parseUnits(amount, decimalsIn),
            path
          );
      ...
      case "second":
        if (amount > 0) {
          ...
          let amountsIn = await routerInstance.getAmountsIn(
            ethers.utils.parseUnits(amount, decimalsOut),
            path
          );
